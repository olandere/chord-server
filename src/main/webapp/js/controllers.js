...
chordApp.filter('musicNotation', function(){
  ...
  $scope.myData.doClick = function (item, event) {
    ...
    var progression =  _.contains($scope.myData.chord, ',');
    if (firstChar.match(/[abcdefg]/i)) {
      url = ($scope.myData.shell ? "/shellchord" : "/chord") +
            (progression ? "progression/" : "/") +
            encodeURIComponent($scope.myData.chord.trim()) + "/" +
            ($scope.myData.fretSpan || 4);
    } else {
      url = "/analyze/" + encodeURIComponent($scope.myData.chord.trim());
    }
    var responsePromise = $http.get(url, {params: {"tuning": $scope.myData.tuning || "E A D G B E","condense": $scope.myData.condense}});
  ...
  var linker = function (scope, element, attrs) {
    ...
    var computeAdjustment = function (numFrets, chord) {
      var minFret = _.min(_.filter(chord, function(v){return v > 0;}));
